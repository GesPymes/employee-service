...
@Component
public class EmployeeMobilePhoneFilter implements FieldFilter<EmployeeFilter> {
    @Override
    public boolean apply(EmployeeFilter employeeFilter) {
        return Objects.nonNull(employeeFilter.getMobilePhone());
    }

    @Override
    public void addSearchCriteria(EmployeeFilter employeeFilter, List<SearchCriteria> searchCriteriaList) {
        searchCriteriaList.add(SearchCriteria.builder().key("mobile_phone").operation(SearchOperation.LIKE).value(employeeFilter.getMobilePhone()).build());
    }
}
