...
@Component
public class EmployeeIdNumberFilter implements FieldFilter<EmployeeFilter> {
    @Override
    public boolean apply(EmployeeFilter employeeFilter) {
        return Objects.nonNull(employeeFilter.getIdNumber());
    }

    @Override
    public void addSearchCriteria(EmployeeFilter employeeFilter, List<SearchCriteria> searchCriteriaList) {
        searchCriteriaList.add(SearchCriteria.builder().key("id_number").operation(SearchOperation.LIKE).value(employeeFilter.getIdNumber()).build());
    }
}
